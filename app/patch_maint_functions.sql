...
--
CREATE FUNCTION public.usp_device_select_all() RETURNS TABLE(x_id_dev character varying, i_typ_dev integer, x_nm_dev character varying, d_srv_bgn timestamp without time zone, d_srv_end timestamp without time zone, n_moe_x real, n_moe_y real, n_moe_z real, f_log boolean, zone_id integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
CREATE FUNCTION public.usp_device_select_by_id(p_device_id character varying) RETURNS TABLE(x_id_dev character varying, i_typ_dev integer, x_nm_dev character varying, d_srv_bgn timestamp without time zone, d_srv_end timestamp without time zone, n_moe_x real, n_moe_y real, n_moe_z real, f_log boolean, zone_id integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY SELECT d.x_id_dev, d.i_typ_dev, d.x_nm_dev, d.d_srv_bgn, d.d_srv_end, d.n_moe_x, d.n_moe_y, d.n_moe_z, d.f_log, d.zone_id
...
--
CREATE FUNCTION public.usp_portable_trigger_add(p_tag_id text, p_name text, p_direction_id integer, p_radius double precision, p_zmin double precision, p_zmax double precision) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    trig_id INT;
...
--
CREATE FUNCTION public.usp_trigger_add(i_dir integer, x_nm_trg character varying, f_ign boolean, i_zn integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
...
BEGIN
    ...
    IF usp_trigger_add.i_zn IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.zones WHERE zones.i_zn = usp_trigger_add.i_zn) THEN
        RAISE EXCEPTION 'Zone ID % does not exist', usp_trigger_add.i_zn;
    END IF;

    -- Insert into triggers and return the new ID
    INSERT INTO public.triggers (X_NM_TRG, I_DIR, F_IGN, I_ZN)
    VALUES (usp_trigger_add.x_nm_trg, usp_trigger_add.i_dir, usp_trigger_add.f_ign, usp_trigger_add.i_zn)
    ...
    -- Log success
    RAISE NOTICE 'Trigger Added: ID = %, Name = %, Direction = %, Ignore = %, Zone = %',
        new_trigger_id, x_nm_trg, i_dir, f_ign, COALESCE(usp_trigger_add.i_zn::text, 'NULL');
...
--
CREATE FUNCTION public.usp_vertex_add(p_i_rgn integer, p_n_x real, p_n_y real, p_n_z real, p_n_ord integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_i_vtx INTEGER;
