...
function startDrawingBuildingL2() {
    ...
    function saveBuildingL2() {
        ...
        .then(data => {
            if (data.message) {
                ...
                document.getElementById("startL3Button").disabled = false;
            } else {
    ...
    function drawL2BorderOnly() {
        let canvas = document.getElementById("zoneCanvas");
        let ctx = canvas.getContext("2d");

        let storedL2Vertices = sessionStorage.getItem("lastL2Vertices");
        if (!storedL2Vertices) {
            console.warn("No stored L2 vertices found.");
            return;
        }

        let vertices = JSON.parse(storedL2Vertices);
        if (vertices.length < 3) {
            console.warn("L2 vertices are not sufficient to draw.");
            return;
        }
