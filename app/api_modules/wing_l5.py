...
"""
Version: 250216 wing_l5.py Version 0P.6B.3B
...
wing_l5 = Blueprint('wing_l5', __name__)

@wing_l5.route('/create_wing_l5', methods=['POST'])
def create_wing_l5():
    data = request.json
    print("📌 Received Payload:", data)  # Debugging

    zone_name = data.get('zone_name')
    parent_zone_id = data.get('parent_zone_id')  # ✅ FIXED: Use correct parent reference
    ...
    try:
        ...
        # ✅ Insert into `zones` with correct type (L5)
        cur.execute("""
            INSERT INTO zones (i_typ_zn, x_nm_zn, i_pnt_zn)
            VALUES (4, %s, %s) RETURNING i_zn
        ...
        conn.commit()
        print(f"🚀 Wing L5 '{zone_name}' successfully created!")
        return jsonify({'message': f'Wing L5 "{zone_name}" created successfully!', 'region_id': new_region_id, 'zone_id': new_zone_id})
